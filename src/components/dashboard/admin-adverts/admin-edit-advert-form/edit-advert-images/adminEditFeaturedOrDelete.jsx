...
import { deleteAdminImageAction, deleteImageAction, updateAdminImageAction } from "@/actions/image-action";
...
const DashboardEditImageToFeaturedOrDelete = ({ advertData }) => {
  ...
  const [state, dispatch] = useFormState(updateAdminImageAction, initialResponse);
  ...
  const handleDelete = async (e) => {
    ...
    try {
      const res = await deleteAdminImageAction(stringifyedImageIds,slugRedirect);
      setImageId([])
    } catch (err) {
      console.log(err);
